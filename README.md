...
This landed as one squashed commit carrying both the configuration and the
95-file reformat, and that commit is listed in `.git-blame-ignore-revs` so the
reformat does not answer for lines it only rewrapped. To make `git blame`
honour it locally:
