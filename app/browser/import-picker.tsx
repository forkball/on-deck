...
export const ImportPicker = clientEntry<ImportPickerProps>(
  import.meta.url,
  function ImportPicker(handle) {
        ...
        >
          {open && (
            ...
            >
              ...
              >
                <div mix={css({ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '12px' })}>
                ...
                <div mix={css({ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '14px' })}>
                  {(data?.candidates ?? []).map((candidate, i) => (
                    ...
                    >
                      <span mix={css({ position: 'relative', display: 'block' })}>
                        {candidate.posterUrl ? (
                          <img
                            ...
                            alt=""
                            mix={css({ width: '100%', aspectRatio: '2 / 3', objectFit: 'cover', borderRadius: '3px' })}
                          />
                        ) : (
                          <span
                            mix={css({
                      ...
                      {candidate.creator && (
                        <span mix={css({ fontSize: '12px', color: '#888', lineHeight: 1.25 })}>{candidate.creator}</span>
                      )}
                    </button>
                  ))}
                </div>
